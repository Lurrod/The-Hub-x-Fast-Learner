...
async def test_accept_does_not_crash_when_fl_role_missing_from_guild():
    ...
    applicant, members_role, _fl_role, _fl_open = await _run_accept_with_tier(
        queue_label="Pro Queue",
        fl_role_name="FL PRO",
...
async def test_welcome_apply_open_grants_fl_hub_and_fl_open_roles():
    ...
    review_view = ApplicationReviewView(db=db)
    view = WelcomeView(db=db, review_view=review_view)
    inter = _welcome_interaction_with_guild(fl_open_in_guild=True)
...
async def test_welcome_apply_open_idempotent_when_member_already_has_role():
    """User already has FL OPEN: no re-grant, friendly message."""
    from cogs.applications import WelcomeView

    db: Any = mongomock.MongoClient(tz_aware=True).db
    review_view = ApplicationReviewView(db=db)
    view = WelcomeView(db=db, review_view=review_view)
    inter = _welcome_interaction_with_guild(fl_open_in_guild=True, member_has_fl_open=True)
...
async def test_welcome_apply_open_warns_when_fl_open_missing_from_guild():
    ...
    review_view = ApplicationReviewView(db=db)
    view = WelcomeView(db=db, review_view=review_view)
    inter = _welcome_interaction_with_guild(fl_open_in_guild=False)
